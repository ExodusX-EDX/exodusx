...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete ExodusX database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no ExodusX datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old ExodusX datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected ExodusX 0.7 datadir."
    ;;
  3)
    echo "Detected ExodusX pre-0.8 datadir."
    ;;
  4)
    echo "Detected ExodusX 0.8 datadir."
    ;;
esac
