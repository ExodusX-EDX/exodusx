...
# exodusx_hash
RUN git clone https://github.com/exodusx/exodusx_hash
RUN cd exodusx_hash && python3 setup.py install
...
RUN groupadd -g ${GROUP_ID} exodusx
RUN useradd -u ${USER_ID} -g exodusx -s /bin/bash -m -d /exodusx exodusx
...
RUN mkdir /exodusx-src && \
  mkdir -p /cache/ccache && \
  mkdir /cache/depends && \
  mkdir /cache/sdk-sources && \
  chown $USER_ID:$GROUP_ID /exodusx-src && \
