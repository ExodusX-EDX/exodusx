...
ExodusX Core is a multithreaded application, and deadlocks or other multithreading bugs
...
However, in open source software such as ExodusX Core, where everyone uses
...
pay attention to for reviewers of ExodusX Core code.

General ExodusX Core
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: ExodusX Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
be sent upstream but bugfixes may also be prudent to PR against ExodusX Core so that they can be integrated
