...
*Setup instructions for a Gitian build of ExodusX Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the ExodusX
...
- [Building ExodusX Core](#building-exodusx-core)
...
Clone the git repositories for ExodusX Core and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/exodusx/exodusx
git clone https://github.com/exodusx/gitian.sigs.git
```
...
in the ExodusX Core repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building ExodusC Core
----------------

To build ExodusX Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the ExodusX Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/exodusx/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/exodusx/exodusx
...
For example:
```bash
URL=https://github.com/crowning-/exodusx.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit exodusx=${COMMIT} --url exodusx=${URL} ../exodusx/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit exodusx=${COMMIT} --url exodusx=${URL} ../exodusx/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit exodusx=${COMMIT} --url exodusx=${URL} ../exodusx/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the exodusx git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=bionic on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../exodusx/contrib/gitian-descriptors/*|sort|uniq )
...
cd /some/root/path/
git clone https://github.com/exodusx/exodusx-detached-sigs.git

BTCPATH=/some/root/path/exodusx
SIGPATH=/some/root/path/exodusx-detached-sigs

./bin/gbuild --url exodusx=${BTCPATH},signature=${SIGPATH} ../exodusx/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/exodusx-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/exodusx-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/exodusx-osx-build.assert
```
...
[exodusx/gitian.sigs](https://github.com/exodusx/gitian.sigs/) repository, or if that's not possible to create a pull
request.
